function NoteslistAssistant(args) {
	/* this is the creator function for your scene assistant object. It will be passed all the 
	   additional parameters (after the scene name) that were passed to pushScene. The reference
	   to the scene controller (this.controller) has not be established yet, so any initialization
	   that needs the scene controller should be done in the setup function below. */
	Mojo.Log.info("NoteslistAssistant Constructor params %j", args);
	
		this.launchSearchString = (args && args.search) ? args.search : '';

	this.notes = [];	// main array of notes
...
NoteslistAssistant.prototype.headerMenuTap = function (event) {
	this.controller.popupSubmenu({
		...
		toggleCmd: MyAPP.prefs.sortOrder,
		title: $L("Sort by"),
...
NoteslistAssistant.prototype.listDelete = function (event) {
	...
	this.someNotes.splice(event.index, 1);
	//this.activate();
};
...
NoteslistAssistant.prototype.listTap = function (event) {
	...
	//Mojo.Log.info("Event: %j", Object.toJSON(event.item));
	//debugObject(event.currentTarget, 'noFuncs');
	...
	if (className === 'note-details-link') {
		this.controller.stageController.pushScene('editnote', 
			this.someNotes[event.index].value);
		...
		note = this.someNotes[event.index];
...
NoteslistAssistant.prototype.doFilter = function (filterstring) {
	var listWidget = this.controller.get("noteslist");
	listWidget.mojo.open();
	//listWidget.filterField.setText(filterstring);
	//this.filterFunction (filterstring, listWidget, 0, 20);
	Mojo.Event.send(listWidget, Mojo.Event.filter, {filterString: filterstring});
...
NoteslistAssistant.prototype.filterFunction = function (filterString, listWidget, offset, count) {
	...
	if (filterString !== '') {
		...
		for (i = 0; i < len; i++) {
			...
			if (s.tags.toUpperCase().indexOf(filterString.toUpperCase()) >=0) {
				//Mojo.Log.info("Found string in tags", i);
				this.someNotes.push(s);
			}
		}
	}
...
NoteslistAssistant.prototype.activate = function(event) {
	...
	Mojo.Log.info("Activate event %j", event);
		...
		if (response) {
			...
			for (i = 0; i < response.length; i++) {
				if (response[i].deleted === 'False') {
					...
					//response[i].preview = response[i].note.slice(firstCR + 1).replace(/<br\s*\/*>/gi, '');
					//response[i].previewFormatted = response[i].note.slice(firstCR + 1).replace(/\n/g, '<br/>');
					//Mojo.Log.info("Before", i,response[i].title);
					response[i].preview = (firstCR !== -1) ? Mojo.Format.runTextIndexer(response[i].note.slice(firstCR + 1, firstCR + 60)).replace(/<br\s*\/*>/gi, '') : "";
					//response[i].previewFormatted = (firstCR !== -1) ? Mojo.Format.runTextIndexer(response[i].note.slice(firstCR + 1)).replace(/\n/g, '<br/>') : "";
					//Mojo.Log.info("After");
